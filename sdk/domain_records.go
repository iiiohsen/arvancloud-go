...
type DomainRecordCreateOptions struct {
	...
	TTLSec   int              `json:"ttl_sec,omitempty"` // 0 is not accepted by Arvancloud, so can be omitted
...
// DomainRecordType constants start with RecordType and include Arvancloud API Domain Record Types
